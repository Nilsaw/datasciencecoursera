...
barplot(steps_per_day$steps,main="Histogram of the total number of steps taken each day" ,names.arg=steps_per_day$date, xlab="Dates", ylab="Steps",col=c("blue","red"),beside=TRUE)
```
...
plot(steps_per_interval, xlab="Intervals", ylab="Steps taken",col="red", main="Time series plot for intervals",type="l")
...
```{r}
activty_data <- merge(activty_data, steps_per_interval, by="interval", suffixes=c("",".y"))
...
weekday_type<-"Weekday"
    steps_per_type <- aggregate(steps ~ interval,data=activty_data,subset=activty_data$daytype==weekday_type,FUN=mean)
